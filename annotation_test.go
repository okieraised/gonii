...
func TestSegmentation_Annotation(t *testing.T) {
	...
	//segments := [][]int{seg1, seg2}
	//voxels := rd.GetNiiData().GetVoxels()
	//colorCode := 1
	//for _, segment := range segments {
	//	for idx, segmentLength := range segment {
	//		if idx%2 == 0 {
	//
	//		} else {
	//
	...
	//inflatedSeg := make([]int, len(rd.GetNiiData().GetVoxels().GetSlice(32, 0)))

	fmt.Println("inflatedSeg", len(inflatedSeg))
